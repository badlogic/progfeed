...
function setupRoutes(app: Express) {
	...
	app.get("/api/download", (req, res) => {
		try {
			...
			if (!feeds.some((feed) => feed.rkey == feedName)) {
				res.status(404).json({ error: "Feed not found" });
				return;
			}

			const filePath = storage.getFilePath(feedName);
...
function startFirehoseWorker() {
	const isDebugMode = process.execArgv.some(
		(arg) => arg.startsWith("--inspect-brk=") || arg.startsWith("--inspect=")
	);
